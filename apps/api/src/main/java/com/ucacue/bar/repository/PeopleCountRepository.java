...
public interface PeopleCountRepository extends JpaRepository<PeopleCountEntity, Long> {
    ...
    @Query("select p from PeopleCountEntity p where p.tenantId = :tenantId and p.timestamp between :from and :to order by p.timestamp asc")
    List<PeopleCountEntity> findBetweenByTenantId(@Param("tenantId") Long tenantId,
                                                    @Param("from") LocalDateTime from,
                                                    @Param("to") LocalDateTime to);

    void deleteByTenantIdAndTimestampBefore(Long tenantId, LocalDateTime threshold);
}
