...
public class LoyaltyService {
    ...
    public int awardForOrder(OrderEntity order) {
        ...
        Long tenantId = order.getTenantId() != null ? order.getTenantId()
                : tenantContextResolver.resolveCurrent().getId();
        int points = calculatePoints(tenantId, order.getTotal());
        if (points <= 0) {
            return 0;
        }
        LoyaltyBalanceEntity balance = balanceRepository.findByTenantIdAndUserId(tenantId, order.getUser().getId())
            .orElseGet(() -> initBalance(tenantId, order.getUser()));
    ...
    @Transactional
    public void revokeForOrder(OrderEntity order) {
        if (order.getUser() == null || order.getLoyaltyPointsAwarded() == null || order.getLoyaltyPointsAwarded() <= 0) {
            order.setLoyaltyPointsAwarded(0);
            return;
        }

        int points = order.getLoyaltyPointsAwarded();
        Long tenantId = order.getTenantId() != null ? order.getTenantId()
                : tenantContextResolver.resolveCurrent().getId();
        LoyaltyBalanceEntity balance = balanceRepository.findByTenantIdAndUserId(tenantId, order.getUser().getId())
            .orElseGet(() -> initBalance(tenantId, order.getUser()));
        balance.setPoints(Math.max(0, balance.getPoints() - points));
        balanceRepository.save(balance);
        ...
        ledger.setUser(order.getUser());
        ledger.setDelta(-points);
        ledger.setReason("ORDER_CANCELLED:" + order.getId());
        ledgerRepository.save(ledger);

        order.setLoyaltyPointsAwarded(0);
        emitRealtime(order.getUser().getId(), balance.getPoints());
        log.info("Revoked {} loyalty points from user {} for order {}", points, order.getUser().getEmail(),
                order.getId());
    }
    ...
    public LoyaltyBalanceResponse getBalance(Long userId) {
        ...
        LoyaltyBalanceEntity balance = balanceRepository.findByTenantIdAndUserId(tenantId, userId)
            .orElse(null);
    ...
    public List<LoyaltyLedgerEntryResponse> getLedger(Long userId) {
        Long tenantId = tenantContextResolver.resolveCurrent().getId();
        return ledgerRepository.findByTenantIdAndUserIdOrderByCreatedAtDesc(tenantId, userId).stream()
    ...
    private int calculatePoints(Long tenantId, BigDecimal total) {
        BigDecimal factor = resolvePointsFactor(tenantId);
        return total.multiply(factor)
            .setScale(0, RoundingMode.DOWN)
            .intValue();
    }

    private BigDecimal resolvePointsFactor(Long tenantId) {
        return settingRepository.findByTenantIdAndKey(tenantId, SETTING_POINTS_PER_DOLLAR)
            .map(SettingEntity::getValue)
            .map(value -> {
                try {
                    return new BigDecimal(value);
                } catch (NumberFormatException ex) {
                    throw new IllegalStateException("Configuracion points_per_dollar invalida: " + value, ex);
                }
            })
    ...
    private LoyaltyBalanceEntity initBalance(Long tenantId, UserEntity user) {
    ...
    private void emitRealtime(Long userId, int points) {
        try {
            ...
            log.warn("Unable to publish loyalty update for user {}", userId, ex);
        }
    }
}
