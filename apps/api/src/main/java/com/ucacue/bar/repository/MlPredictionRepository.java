...
public interface MlPredictionRepository extends JpaRepository<MlPredictionEntity, Long> {
    ...
    @Query("select p from MlPredictionEntity p where p.tenantId = :tenantId and p.order.id = :orderId and p.predictionType = :type order by p.createdAt desc")
    Optional<MlPredictionEntity> findLatestByTenantIdAndOrderAndType(@Param("tenantId") Long tenantId,
